...
pub fn round_layout(tree: &mut impl RoundTree, node_id: NodeId) {
    ...
    fn round_layout_inner(tree: &mut impl RoundTree, node_id: NodeId, cumulative_x: f32, cumulative_y: f32) {
        ...
        let layout = round_single_layout(&unrounded_layout, cumulative_x, cumulative_y);
...
/// PATCH: the rounded (final) layout of a single node whose parent's absolute position is
/// (`cumulative_x`, `cumulative_y`), as computed by [`round_layout`]. Lets a tree round
/// incrementally (only subtrees whose layout or position changed).
pub fn round_single_layout(unrounded_layout: &Layout, cumulative_x: f32, cumulative_y: f32) -> Layout {
    let unrounded_layout = *unrounded_layout;
    ...
    #[cfg(feature = "content_size")]
    round_scrollable_overflow_rect_single(
        &mut layout,
...
#[cfg(feature = "content_size")]
#[inline(always)]
fn round_scrollable_overflow_rect_single(
    layout: &mut Layout,
