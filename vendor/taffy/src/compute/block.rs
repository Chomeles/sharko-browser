...
struct BlockItem {
    ...
    /// PATCH: whether `max-width` is a percentage (see `determine_content_based_container_width`)
    max_width_is_percentage: bool,
    ...
    tree.child_ids(node)
        ...
        .map(|(order, (child_node_id, child_style))| {
            ...
            BlockItem {
                ...
                max_width_is_percentage: {
                    let w = child_style.max_size().width;
                    let a = w.maybe_resolve(Some(0.0), |val, basis| tree.calc(val, basis));
                    let b = w.maybe_resolve(Some(1000.0), |val, basis| tree.calc(val, basis));
...
fn determine_content_based_container_width(
    ...
    for item in items.iter().filter(|item| item.position != Position::Absolute) {
        ...
        // PATCH: compressible replaced elements (css-sizing-3 §5.2.2): for the min-content
        // contribution a percentage `max-width` resolves against zero, so e.g.
        // `<img width=872 style="max-width:100%">` doesn't prop its container open.
        if item.is_replaced && item.max_width_is_percentage && available_width == AvailableSpace::MinContent {
            known_dimensions.width = Some(0.0f32.maybe_max(item.min_size.width));
        }
