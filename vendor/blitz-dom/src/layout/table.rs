...
pub(crate) fn build_table_context(
    ...
    // PATCH: anonymous full-row cells may have registered a column before any real one.
...
pub(crate) fn collect_table_cells(
    ...
    match display.inside() {
        ...
        | DisplayInside::Flex
        | DisplayInside::Grid => {
            // PATCH: CSS 2.1 §17.2.1 "anonymous table objects". A non-table box inside a
            // table (e.g. `<div style="display:table"><h2>..</h2></div>`, a common layout
            // pattern) must be wrapped in an anonymous row + cell instead of being dropped.
            // Approximation: the box becomes its own cell spanning a full row (or, inside
            // a row, an ordinary cell).
            let in_row = *col > 0 || rows.last().is_some_and(|r| {
                doc.nodes[node_id].parent == Some(r.node_id)
            });
            let node = &mut doc.nodes[node_id];
            let Some(stylo_style) = node.primary_styles() else { return };
            let mut style = stylo_taffy::to_taffy_style(&stylo_style);
            drop(stylo_style);
            if in_row {
                style.grid_column = taffy::Line {
