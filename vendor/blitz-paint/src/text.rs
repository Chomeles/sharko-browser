...
pub(crate) fn draw_inline_backgrounds<'a>(
    ...
    for line in lines {
        ...
        for item in line.items() {
            // PATCH: paint the border+padding spacer boxes of inline elements.
            if let PositionedLayoutItem::InlineBox(ibox) = item {
                if ibox.id & blitz_dom::INLINE_EDGE_SPACER == 0
                    || ibox.id & blitz_dom::INLINE_MARGIN_SPACER != 0
                {
                    continue;
                }
                let node_id = NodeId::from_u64(ibox.id & !blitz_dom::INLINE_EDGE_SPACER);
                let Some(styles) = doc.get_node(node_id).and_then(|node| node.primary_styles())
                else {
                    continue;
                };
                ...
                let y0 = baseline - line_metrics.ascent as f64;
                let y1 = baseline + line_metrics.descent as f64;
                let rect = Rect::new(ibox.x as f64, y0, (ibox.x + ibox.width) as f64, y1);
