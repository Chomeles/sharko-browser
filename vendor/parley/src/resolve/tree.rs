...
impl<B: Brush> TreeStyleBuilder<B> {
    ...
    pub(crate) fn push_uncommitted_text(&mut self, is_span_last: bool) {
        ...
        let span_text = match self.white_space_collapse {
            ...
            WhiteSpaceCollapse::Collapse => {
                ...
                // PATCH: CSS white-space collapsing ignores inline element boundaries.
                // Leading spaces are only removed at the start of the inline formatting
                // context (`is_span_first` is no longer set per span) or after a
                // collapsible space, and trailing spaces only at its end (`finish`).
                // Only collapsible (ASCII) white space is trimmed: `trim_start()` would
                // also remove U+00A0 (`&nbsp;`).
                let collapsible = |c: char| c.is_ascii_whitespace();
                if self.is_span_first
                    || (self.last_item_kind == ItemKind::TextRun
                        && self.text.chars().last().is_some_and(collapsible))
                {
                    span_text = span_text.trim_start_matches(collapsible);
                }
                if is_span_last {
                    span_text = span_text.trim_end_matches(collapsible);
                }
    ...
    pub(crate) fn push_style_span(&mut self, style: ResolvedStyle<B>) {
        ...
        // PATCH: a new span is not a new line (see `push_uncommitted_text`).
    }
    ...
    pub(crate) fn pop_style_span(&mut self) {
        // PATCH: the end of a span is not the end of the paragraph: keep its trailing
        // space (`<b>word </b>next`).
        self.push_uncommitted_text(false);
