...
- **Unsupported, or stubbed as absent or `null`:** import maps (`<script type=importmap>` is ignored), iframes and nested browsing contexts (`contentWindow` is `null`), Worker/SharedWorker, EventSource, IndexedDB, Notification, WebGL, WebAssembly streaming helpers, `OffscreenCanvas`. Canvas 2D and media elements are inert stubs.
